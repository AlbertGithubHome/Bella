#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Date     : 2018-04-16 17:04:17
# @Author   : Albert Shi
# @Link     : http://blog.csdn.net/albertsh
# @Github   : https://github.com/AlbertGithubHome
__author__ = 'AlbertS'
# @Subject  : level 3 of python challenge
# 
# 思路：本来挺简单的一关，结果花了20分钟，其中写逻辑代码3分钟，写正则表达式17分钟，
#       想想还是自己的正则表达式不过关，耽误了太多的时间
#       第一步还是利用requests库发送get请求，取到全部网页内容，
#       然后找到其中注释的内容，也就是<!--  ... -->之间的代码，通过string.find('<!--')找到注释开始的内容
#       然后写正则表达式[^A-Z][A-Z]{3}([a-z])[A-Z]{3}[^A-Z]，也就是找到这些满足条件小写字母，
#       这些小写字母前后都恰好有3个大写字母，要注意前后第4个位置不能是大写字母
#       把这些字母连起来就是linkedlist，访问linkedlist.html发现页面上有文字linkedlist.php
#       于是改为访问链接linkedlist.php
#       
#       由于自己对re.match和re.findall不了解，所以耽误了时间，特此记录
#       
#       re.match ：从首字母开始开始匹配，string如果包含pattern子串，则匹配成功，返回Match对象，
...
# entrance url
target_url = 'http://www.pythonchallenge.com/pc/def/equality.html'
...
def find_next_url(code_str):
    print(code_str)
    letter_list = re.findall(r'[^A-Z][A-Z]{3}([a-z])[A-Z]{3}[^A-Z]', code_str)
