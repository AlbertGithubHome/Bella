    ...
    def next(self):
        ...
        if not self.position:
            if prev_close < prev_ma5 and close > ma5:
                self.log(f'满足买入条件：昨日收盘 {prev_close:.2f} < MA5 {prev_ma5:.2f} 且 今日收盘 {close:.2f} > MA5 {ma5:.2f}')
                self.buy()
        else:
            if prev_close < prev_ma5 and close < ma5:
                self.log(f'满足卖出条件：昨日收盘 {prev_close:.2f} < MA5 {prev_ma5:.2f} 且 今日收盘 {close:.2f} < MA5 {ma5:.2f}')
                self.sell()
